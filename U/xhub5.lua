...
local function applySpeedBoost()
    ...
    humanoid.WalkSpeed = SpeedBoostConfig.OriginalSpeed * SpeedBoostConfig.Multiplier
end
...
local Tabs = {
    ...
    Speed = Window:AddTab("ความเร็ว", "zap"),
...
-- SPEED BOOST TAB
-- ═══════════════════════════════════════════════
local SpeedBoostBox = Tabs.Speed:AddLeftGroupbox("Speed Boost ความเร็ว", "zap")
SpeedBoostBox:AddToggle("SpeedBoostEnabled", {
    Text = "เปิดใช้งาน Speed Boost",
    ...
    Callback = function(v)
        ...
        if v then
            ...
            if char then
                local hum = char:FindFirstChildOfClass("Humanoid")
                if hum then
                    hum.WalkSpeed = SpeedBoostConfig.OriginalSpeed
                end
            end
        end
    end
...
SpeedBoostBox:AddSlider("SpeedMultiplier", {
    Text = "ตัวคูณความเร็ว",
    Default = SpeedBoostConfig.Multiplier,
    Min = 1,
    Max = 5,
    Rounding = 2,
    Callback = function(v)
        SpeedBoostConfig.Multiplier = v
...
SpeedBoostBox:AddButton("รีเซ็ตเป็นค่าเริ่มต้น", function()
    SpeedBoostConfig.Multiplier = 1.5
    if SpeedBoostConfig.Enabled then
        applySpeedBoost()
    end
    Library:Notify({ Title = "Speed Boost", Content = "รีเซ็ตเป็นค่าเริ่มต้น 1.5x", Duration = 2 })
end)
...
-- Keybind for quick toggle (optional - RightShift)
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if gameProcessed then return end
    if input.KeyCode == Enum.KeyCode.RightShift then
        SpeedBoostConfig.Enabled = not SpeedBoostConfig.Enabled
        if SpeedBoostConfig.Enabled then
            applySpeedBoost()
            Library:Notify({ Title = "Speed Boost", Content = "เปิดใช้งาน " .. string.format("%.2fx", SpeedBoostConfig.Multiplier), Duration = 2 })
        else
            local char = LocalPlayer.Character
            if char then
                local hum = char:FindFirstChildOfClass("Humanoid")
            ...
            Library:Notify({ Title = "Speed Boost", Content = "ปิดใช้งาน", Duration = 2 })
        end
    end
end)
