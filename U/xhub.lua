...
-- =======================================
Library.Scheme.AccentColor = Color3.fromRGB(120, 180, 100)
Library.Scheme.BackgroundColor = Color3.fromRGB(12, 18, 10)
Library.Scheme.MainColor = Color3.fromRGB(20, 28, 18)
Library.Scheme.OutlineColor = Color3.fromRGB(80, 130, 60)
Library.Scheme.FontColor = Color3.fromRGB(230, 245, 220)
-- =======================================
...
local function CreateXHUBToggleMenu(IconId)
    ...
    Gradient.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, Color3.fromRGB(40, 60, 30)),
        ColorSequenceKeypoint.new(0.3, Color3.fromRGB(120, 180, 100)),
        ColorSequenceKeypoint.new(0.5, Color3.fromRGB(90, 150, 70)),
        ColorSequenceKeypoint.new(0.7, Color3.fromRGB(120, 180, 100)),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(40, 60, 30))
    })
...
-- =======================================
-- ═══════════════════════════════════════════════
-- INVISIBLE CONFIG (CFrame Offset Method)
-- ═══════════════════════════════════════════════
...
local function startInvisible()
    ...
    invisConn = RunService.Heartbeat:Connect(function()
        ...
        local camOff = invisHumanoid.CameraOffset
        invisRoot.CFrame = cf * CFrame.new(0, -200000, 0)
        invisHumanoid.CameraOffset = Vector3.new(camOff.X, camOff.Y + 200000, camOff.Z)
...
-- ═══════════════════════════════════════════════
-- ═══════════════════════════════════════════════
-- MOVEMENT TAB: INVISIBLE + NOCLIP + SPEED BOOST
-- ═══════════════════════════════════════════════
local InvisibleBox = Tabs.Movement:AddLeftGroupbox("ล่องหน", "eye-off")
InvisibleBox:AddToggle("InvisibleEnabled", {
    Text = "เปิดใช้งานล่องหน",
    Default = InvisibleConfig.Enabled,
    Callback = function(v)
        InvisibleConfig.Enabled = v
        if v then
            startInvisible()
            Library:Notify({ Title = "ล่องหน", Content = "เปิดใช้งานแล้ว", Duration = 2 })
        else
            stopInvisible()
            Library:Notify({ Title = "ล่องหน", Content = "ปิดใช้งานแล้ว", Duration = 2 })
        end
    end
})
...
InfoBox:AddLabel("สคริปต์: UNICORN HUB")
InfoBox:AddLabel("เวอร์ชัน: 1.0")
InfoBox:AddLabel("เกม: Violence District")

CreditsBox:AddLabel("ผู้พัฒนา: i am")
CreditsBox:AddLabel("เซิร์ฟเวอร์: UNICORN HUB")
